...
__all__ = (
    ...
    'LightweightCrossFusion', 'CrossModalAttention', 'ECAAttention', 'AdaptiveWeightedFusion','StableCrossFusion',
)
...
class ChannelAttention(nn.Module):
    """
    通道注意力模块 (支持 reduction 参数)
    - 输入: B x C x H x W
    - 输出: B x C x H x W (通道加权后)
    """
...
class LightweightCrossFusion(nn.Module):
    """
    Lightweight Cross-Modal Fusion module optimized for memory efficiency and training stability
    """
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, reduction=8):
        super(LightweightCrossFusion, self).__init__()
        self.c1 = c1
        self.c2 = c2
        actual_channels = c1 // 2
        
        # Simplified attention mechanism - only channel attention
        self.channel_attention = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(actual_channels, actual_channels // reduction, 1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(actual_channels // reduction, actual_channels, 1, bias=False),
            nn.Sigmoid()
        )
        
        # Lightweight fusion with fewer parameters
        ...
        # Simple learnable weights with proper initialization
        self.modal_weight = nn.Parameter(torch.tensor([0.3, 0.7]))
        
        # Output projection
        self.output_proj = nn.Conv2d(c2, c2, k, s, autopad(k, p), groups=g, bias=False)
        
        self._init_weights()
    
    def _init_weights(self):
        """Initialize weights to prevent training instability"""
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
        ...
        # Initialize modal weights to prevent extreme values
        nn.init.constant_(self.modal_weight, 0.5)
    
    def forward(self, x):
        # Split input into RGB and IR features
        if isinstance(x, (list, tuple)):
            rgb_feat, ir_feat = x
        else:
            # Handle single tensor input (concatenated features)
            split_size = x.shape[1] // 2
            rgb_feat = x[:, :split_size]
            ir_feat = x[:, split_size:]
        
        # Apply channel attention to each modality
        rgb_att = self.channel_attention(rgb_feat)
        ir_att = self.channel_attention(ir_feat)
        
        # Weighted feature enhancement
        weighted_rgb = rgb_feat * rgb_att
        weighted_ir = ir_feat * ir_att
        
        # Simple weighted fusion
        fused_features = torch.cat([weighted_rgb, weighted_ir], dim=1)
        
        # Lightweight fusion convolution
...
class StableCrossFusion(nn.Module):
    """
    Stable Cross-Modal Fusion with gradient clipping and numerical stability
    """
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, reduction=16):
        super(StableCrossFusion, self).__init__()
        self.c1 = c1
        self.c2 = c2
        actual_channels = c1 // 2
        
        # Single-scale attention to reduce memory
        ...
        # Simple fusion with residual connection
    ...
    def forward(self, x):
        ...
        # Apply attention
        rgb_att = self.attention(rgb_feat)
        ir_att = self.attention(ir_feat)
        
        # Enhanced features
        enhanced_rgb = rgb_feat * rgb_att
        enhanced_ir = ir_feat * ir_att
        
        # Concatenate and fuse
        fused = torch.cat([enhanced_rgb, enhanced_ir], dim=1)
        fused = self.fusion(fused)
        
        # Residual connection if possible
        if self.use_residual:
            # Ensure same spatial dimensions
            if x.shape[2:] == fused.shape[2:]:
                fused = fused + x
...
# Keep the original EnhancedCrossFusion for reference but rename it
class EnhancedCrossFusionOriginal(nn.Module):
    """
    Original EnhancedCrossFusion module (kept for reference)
    WARNING: This module may cause high GPU memory usage and training instability
    """
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, reduction=16,
                 num_heads=8, pool_scales=[1, 2, 4], use_eca=True):
        super(EnhancedCrossFusionOriginal, self).__init__()
        self.c1 = c1
        self.c2 = c2
        self.pool_scales = pool_scales
        self.use_eca = use_eca
        self.multi_scale_pools = nn.ModuleList([
            nn.AdaptiveAvgPool2d((h, w)) for h, w in [(16, 16), (8, 8), (4, 4)]
        ])
        actual_channels = c1 // 2
        self.channel_attention = ChannelAttention(actual_channels, reduction)
        self.spatial_attention = SpatialAttention()
        self.cross_modal_attention = CrossModalAttention(actual_channels, num_heads)
        groups = max(1, actual_channels // 4)
        self.fusion_conv = nn.Sequential(
            nn.Conv2d(c1, actual_channels, 3, 1, 1, groups=groups, bias=False),
            nn.BatchNorm2d(actual_channels),
            nn.SiLU(),
            nn.Conv2d(actual_channels, c2, 1, 1, bias=False),
            nn.BatchNorm2d(c2),
            nn.SiLU()
        )
        self.output_proj = nn.Conv2d(c2, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.modal_weight = nn.Parameter(torch.tensor([0.5, 0.5]))
        self.scale_weights = nn.Parameter(torch.ones(len(self.multi_scale_pools)))
        if use_eca:
    ...
    def _init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                nn.init.constant_(m.bias, 0)
        nn.init.constant_(self.modal_weight, 0.5)
        nn.init.constant_(self.scale_weights, 1.0)
    ...
    def forward(self, x):
        ...
        for i, pool in enumerate(self.multi_scale_pools):
            pooled_rgb = pool(rgb_feat)
            pooled_ir = pool(ir_feat)
            multi_scale_features.append(torch.cat([pooled_rgb, pooled_ir], dim=1))
        
        # Attention mechanisms
        rgb_att = self.channel_attention(rgb_feat)
        ir_att = self.channel_attention(ir_feat)
        
        spatial_att = self.spatial_attention(torch.cat([rgb_feat, ir_feat], dim=1))
        
        cross_att = self.cross_modal_attention(rgb_feat, ir_feat)
        
        # Weighted features
        weighted_rgb = rgb_feat * rgb_att * spatial_att[:, :rgb_feat.shape[1]]
        weighted_ir = ir_feat * ir_att * spatial_att[:, rgb_feat.shape[1]:]
        
        # Cross-modal attention
        weighted_rgb = weighted_rgb * cross_att[:, :rgb_feat.shape[1]]
        weighted_ir = weighted_ir * cross_att[:, rgb_feat.shape[1]:]
        
        # Multi-scale fusion
        multi_scale_fused = torch.stack(multi_scale_features, dim=0)
        multi_scale_fused = multi_scale_fused * self.scale_weights.view(-1, 1, 1, 1, 1)
        multi_scale_fused = multi_scale_fused.sum(dim=0)
...
class CrossModalAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        ir_seq = ir_feat.flatten(2).transpose(1, 2)    # B, HW, C
        
        # 计算QKV
        qkv_rgb = self.qkv(rgb_feat).reshape(B, 3, self.num_heads, self.head_dim, H*W).permute(1, 0, 2, 4, 3)
        qkv_ir = self.qkv(ir_feat).reshape(B, 3, self.num_heads, self.head_dim, H*W).permute(1, 0, 2, 4, 3)
        
        q_rgb, k_rgb, v_rgb = qkv_rgb[0], qkv_rgb[1], qkv_rgb[2]
        q_ir, k_ir, v_ir = qkv_ir[0], qkv_ir[1], qkv_ir[2]
        
        # 交叉注意力: RGB关注IR
        attn_rgb = (q_rgb @ k_ir.transpose(-2, -1)) * self.scale
        attn_rgb = attn_rgb.softmax(dim=-1)
        rgb_out = (attn_rgb @ v_ir).transpose(-2, -1).reshape(B, C, H, W)
        
        # 交叉注意力: IR关注RGB
        attn_ir = (q_ir @ k_rgb.transpose(-2, -1)) * self.scale
        attn_ir = attn_ir.softmax(dim=-1)
        ir_out = (attn_ir @ v_rgb).transpose(-2, -1).reshape(B, C, H, W)
        
        # 融合输出
        fused_out = rgb_out + ir_out
...
class ECAAttention(nn.Module):
    ...
    def __init__(self, gamma=2, b=1):
        ...
        self.gamma = gamma
        self.b = b
        self.t = int(abs(math.log2(64) / self.gamma + self.b / self.gamma))
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.conv = nn.Conv1d(1, 1, kernel_size=self.t, padding=(self.t - 1) // 2, bias=False)
        self.sigmoid = nn.Sigmoid()
...
class AdaptiveWeightedFusion(nn.Module):
    ...
    def forward(self, x1, x2):
        # 计算自适应权重
        avg_out = self.avg_pool(torch.cat([x1, x2], dim=1))
        max_out = self.max_pool(torch.cat([x1, x2], dim=1))
        attention = self.fc(avg_out + max_out)
        attention = self.sigmoid(attention)
        
        # 加权融合
        weight1 = attention[:, :x1.size(1), :, :]
        weight2 = attention[:, x1.size(1):, :, :]
